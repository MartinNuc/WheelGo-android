...
public class GenericHandler
{
																										...
																										// IllegalAccessException
	{
		...
		try
		{
			...
			if ( wrappedResultClass != null )
			{
				...
				XMLReflectionUtil.invokeSetter ( result, resultName, wrappedResultObject );
			}
	...
	public void startElement ( String originalName )
	{
		...
		else
		{
			...
			try
			{
				fieldClass = ReflectionHelper.getFieldClass ( object.getClass ( ), name );
			}
			...
			if ( fieldClass.equals ( List.class ) || fieldClass.isArray ( ) )
			{
				...
				{
					listObject = (List) XMLReflectionUtil.invokeGetter ( object, name );
			...
			else
			{
				...
				try
				{
					...
					else
					{
						...
						XMLReflectionUtil.invokeSetter ( parentObject, name, fieldObject );
					}
				}
				catch ( Exception except )
				{
	...
	public void endElement ( String originalName )
	{
		...
		if ( this.arrayFieldData != null && this.arrayFieldData.size ( ) > 0 )
		{
			...
			if ( this.xmlPath.size ( ) < afi.arrayFieldDepth )
			{
				...
				XMLReflectionUtil.invokeSetter ( object, afi.fieldName, afi.arrayData );
			}
		}
		...
		else
		{
			...
			if ( fieldClass != null && !fieldClass.isArray ( )  && !fieldClass.equals ( List.class ) )
			{
				...
				XMLReflectionUtil.invokeSetter ( object, name, content == null ? null : content.toString ( ) );
			}
			...
			else if ( fieldClass != null && fieldClass.equals ( List.class )  )
			{
				...
				{
					XMLReflectionUtil.invokeSetter ( object, name,  content.toString ( ) );
				}
				// if its the list of some business class,
				//field class is list now add the object generated so far to the list
				else
				{
					XMLReflectionUtil.invokeSetter ( object, name, fieldObject );
				}
